...
/**
 * Wedding guest media upload. Mirrors the per-invite version at
 * /api/contribute/capsule/[token]/upload but resolves on the
 * open guest token. Same three actions: sign (presign R2 PUT),
 * complete (write the keys onto the contribution row), view
 * (re-sign GET for previews).
 */
async function resolveCapsule(token: string) {
  const { prisma } = await import("@/lib/prisma");
  return prisma.memoryCapsule.findUnique({ where: { guestToken: token } });
}
...
export async function POST(
  ...
  const capsule = await resolveCapsule(guestToken);
  ...
  if (
    ...
    return NextResponse.json(
      { error: "Capsule is not accepting contributions." },
      { status: 403 },
    );
  ...
  if (body.action === "sign") {
    ...
    if (!kind || !contentType || !contributionId)
      return NextResponse.json(
        { error: "Missing kind, contentType, or contributionId." },
        { status: 400 },
      );
  ...
  if (body.action === "complete") {
    ...
    const { prisma } = await import("@/lib/prisma");
    // Defensive: confirm the contribution actually belongs to
    // this capsule before letting an arbitrary ID overwrite media.
