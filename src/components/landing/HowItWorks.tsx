export function HowItWorks() {
  return (
    <section id="how" className="bg-cream">
      <div className="mx-auto max-w-[1080px] px-6 lg:px-14 pt-10 pb-3">
        <p className="text-[11px] font-bold tracking-[0.16em] uppercase text-amber mb-3.5">
          How it works
        </p>
        <h2 className="text-[40px] lg:text-[72px] font-extrabold tracking-[-1.5px] leading-[1.05] text-navy max-w-[820px]">
          Hold onto what
          <br />
          matters &mdash; <span className="font-light italic text-amber">for later.</span>
        </h2>
      </div>
    </section>
  );
}
